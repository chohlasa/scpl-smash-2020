...
```{r}
vector_a <- sample(1:3, 5, replace = T)
```
...
```{r}
vector_b <- sample(1:10, 5, replace = F)
```
...
```{r}
bdays <- sample(1:365, 60, replace = TRUE)
has_duplicate(bdays)
print(bdays)
```
...
```{r}
bdays2 <- sample(1:365,141,replace=T)
num_duplicates(bdays2)
...
```{r}
# Your code here!
bdays3 <- sample(1:3650, 141,replace = TRUE)
num_duplicates(bdays3)
...
```{r}
# Your code here!
replicate(5,sample(1:365,10,replace = T))
